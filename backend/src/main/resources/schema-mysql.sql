...
BEGIN
    ...
    IF v_tech_id IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No available technicians for the requested slot';
    END IF;

    -- 2. Fetch pricing
    SELECT base_price INTO v_base_price 
    FROM pricing 
    WHERE item_name = p_skill AND category = 'Technician';
...
BEGIN
    ...
    SELECT name INTO v_amenity_name FROM amenity WHERE amenity_id = p_amenity_id;

    -- 2. Fetch Pricing
    SELECT base_price INTO v_base_price 
    FROM pricing 
    WHERE item_name = v_amenity_name AND category = 'Amenity';
    ...
    VALUES (CONCAT('BKG-', FLOOR(RAND() * 899999 + 100000)), p_resident_id, p_amenity_id, v_trans_no, p_date, p_slot, p_capacity_booked, 'Confirmed');
