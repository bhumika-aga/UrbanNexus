...
import { CheckCircle2, ChevronLeft, Loader2, Wrench } from "lucide-react";
...
export default function BookTechnician({ onBack }) {
  ...
  if (invoice) {
    return (
      <div className="p-8 max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-orange-200 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 italic mb-2">
        ...
        <div className="bg-gray-50 p-6 rounded-xl text-left font-mono text-sm border border-gray-100">
          <p>
            <strong>Assignment ID:</strong> {invoice.assignment_id}
          </p>
          <p>
            <strong>Crew Member:</strong> {invoice.technician_name}
          </p>
          <p>
            <strong>Date / Slot:</strong>{" "}
            {new Date(invoice.assign_date).toLocaleDateString()} / Slot{" "}
            {invoice.slot}
          </p>
          <p>
            <strong>Base Price:</strong> ₹{invoice.base_price}
          </p>
          <p>
        ...
        <button
          onClick={onBack}
          className="mt-8 bg-gray-900 text-white px-6 py-2 rounded-lg font-bold"
        >
          Return to Dashboard
        </button>
      </div>
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      ...
      <h2 className="text-2xl font-bold mb-6 italic tracking-tighter">
        REQUEST TECHNICAL STAFF
      </h2>
      ...
      >
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-orange-500 text-white py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-orange-600 transition-colors flex items-center justify-center"
        >
          {loading ? (
