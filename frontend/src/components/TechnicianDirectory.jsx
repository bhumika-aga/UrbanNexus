...
export default function TechnicianDirectory() {
  ...
  const fetchTechs = async () => {
    ...
    try {
      ...
      console.error("Staff lookup failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddTech = async (e) => {
    ...
    try {
      ...
      alert("Failed to sign crew: " + err.response?.data?.error);
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6 border-b border-gray-100 flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center space-x-2 bg-gray-900 text-white px-4 py-2 rounded-lg text-sm font-bold"
        >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-md p-8 relative">
            ...
            <h3 className="text-xl font-bold mb-6 italic uppercase tracking-tighter">
              Sign New Pit Crew
            </h3>
            <form onSubmit={handleAddTech} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold uppercase hover:bg-blue-700 transition-colors"
              >
                Deploy to Grid
              </button>
            </form>
