...
import { Wrench, MapPin, User, CheckCircle2, Settings, Loader2 } from 'lucide-react';
import api from '../api';
...
export default function TechnicianDashboard() {
    ...
    const updateStatus = async (id, status) => {
        try {
            ...
            fetchTasks(); // This triggers the SMS via the backend logic
                    ...
                    <div key={task.assignment_id} className="bg-white p-6 rounded-2xl border border-gray-100 flex justify-between items-center shadow-sm">
                        ...
                        <div>
                            {task.status !== 'Resolved' ? (
                                ...
                                <div className="text-green-600 font-bold text-xs uppercase flex items-center"><CheckCircle2 className="w-4 h-4 mr-1" /> Complete</div>
                            )}
                        </div>
                    </div>
                ))}
