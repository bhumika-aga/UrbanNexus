...
export default function TechnicianDashboard() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <h1 className="text-2xl font-bold italic uppercase tracking-tighter border-b border-gray-200 pb-4">
            ...
            >
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">
                  Unit {task.house_block}-{task.house_unit}
                </span>
                <h3 className="text-lg font-bold">{task.resident_name}</h3>
                <p className="text-xs text-gray-500 flex items-center">
                  <MapPin className="w-3 h-3 mr-1" /> Slot {task.slot}
                </p>
              </div>
              <div>
                {task.status !== "Resolved" ? (
                  <button
                    ...
                    className="bg-green-600 text-white px-4 py-2 rounded-lg font-bold text-xs uppercase hover:bg-green-700"
                  >
                    Mark Resolved
                  </button>
                ) : (
                  <div className="text-green-600 font-bold text-xs uppercase flex items-center">
