    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6 border-b border-gray-100 flex justify-between items-center gap-4">
        ...
        <button
          ...
          className="flex items-center space-x-2 bg-orange-100 text-orange-600 px-4 py-2 rounded-lg text-xs font-bold uppercase italic"
        >
            ...
            <tr key={tx.trans_no}>
              ...
              <td className="px-6 py-4 font-bold">
                ...
                <span className="text-[10px] text-gray-400">
                  ({tx.house_block}-{tx.house_unit})
                </span>
              </td>
              <td className="px-6 py-4 font-bold text-blue-600">₹{tx.cost}</td>
              <td className="px-6 py-4 text-[10px] font-bold uppercase">
                <span
                  className={`px-2 py-0.5 rounded ${tx.status === "Paid" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                >
                  {tx.status}
                </span>
              </td>
              <td className="px-6 py-4 text-right">
                {tx.status !== "Paid" && (
                  <button
                    ...
                    className="text-blue-600 hover:underline font-bold text-[10px] uppercase tracking-tighter"
                  >
                    Process Pay
                  </button>
                )}
