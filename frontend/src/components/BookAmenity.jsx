...
export default function BookAmenity({ onBack }) {
    ...
    const fetchFacilities = async () => {
      try {
        ...
        if (response.data.length > 0) {
          setFormData((prev) => ({
            ...prev,
            amenity_id: response.data[0].amenity_id.toString(),
          }));
        }
      } catch (error) {
  ...
  const selectedFacility = facilities.find(
    (f) => f.amenity_id.toString() === formData.amenity_id,
  );
  const isOverCapacity =
    selectedFacility && formData.capacity_booked > selectedFacility.capacity;

  const handleBooking = async (e) => {
    e.preventDefault();
    if (isOverCapacity) {
      alert(
        "Booking blocked: You cannot exceed the facility's capacity limit.",
      );
      return;
    }
  ...
  if (invoice) {
    return (
      <div className="p-8 max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-green-200 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 italic mb-2 uppercase tracking-tighter">
          Reservation Confirmed
        </h2>
        <p className="text-gray-500 mb-6 font-medium">
          Your amenity pass has been generated and synced with the grid.
        </p>
        <div className="bg-gray-50 p-6 rounded-xl text-left font-mono text-sm border border-gray-100">
          ...
          <p>
            <strong>Amenity:</strong> {invoice.amenity_name}
          </p>
          <p>
            <strong>Date / Slot:</strong>{" "}
            {new Date(invoice.booking_date).toLocaleDateString()} / Slot{" "}
            {invoice.slot}
          </p>
          <p>
            <strong>Total Cost (inc. GST):</strong> ₹{invoice.total_with_gst}
          </p>
          <p>
            <strong>Transaction #:</strong> {invoice.trans_no}
          </p>
        </div>
        <button
          onClick={onBack}
          className="mt-8 bg-gray-900 text-white px-8 py-3 rounded-xl font-bold uppercase tracking-widest text-xs"
        >
          Return to Dashboard
        </button>
      </div>
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <button
        onClick={onBack}
        className="flex items-center text-blue-600 mb-6 hover:underline font-bold uppercase text-[10px] tracking-widest"
      >
      ...
      >
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setFormData({ ...formData, amenity_id: e.target.value })
            }
          >
          ...
          {isOverCapacity ? (
            <div className="flex items-center space-x-2 text-red-600 bg-red-50 p-3 rounded-lg border border-red-100">
              ...
              <p className="text-[10px] font-bold uppercase tracking-tighter italic">
                Warning: Capacity exceeded. Max allowed for this facility is{" "}
            ...
            selectedFacility && (
              <div className="flex items-center space-x-2 text-blue-600 bg-blue-50 p-3 rounded-lg border border-blue-100">
                ...
                <p className="text-[10px] font-bold uppercase tracking-tighter italic">
                  Facility capacity: {selectedFacility.capacity} occupants.
                </p>
              </div>
            )
          )}
