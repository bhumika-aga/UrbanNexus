...
Both profiles run `schema.sql` and `data.sql` on startup via Spring SQL initialization. The H2 MySQL compatibility mode
ensures the same DDL and queries run identically on both engines.
...
`AuditService`. The MySQL `LogResidentDeletion` trigger provides a DB-level fallback for direct SQL deletes. The H2 dev
profile handles this via the Java service layer.
...
The H2 database file (`urbannexus_db.mv.db`) is created automatically in `backend/`. The H2 web console is available at
`http://localhost:8080/h2-console`.

On first start, `schema.sql` creates all tables and `data.sql` seeds 26 residents, 16 technicians, 5 amenities, and
pricing.
